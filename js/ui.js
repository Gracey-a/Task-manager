...
export function getFilteredTasks() {
    let filtered = tasks.filter(t => { if (currentFilter === "active") return !t.completed; if (currentFilter === "completed") return t.completed; return true; });
    if (searchQuery) { const q = searchQuery.toLowerCase(); filtered = filtered.filter(t => t.title.toLowerCase().includes(q) || t.description.toLowerCase().includes(q) || (t.tags && t.tags.some(tag=>tag.toLowerCase().includes(q)))); }
...
export function renderStats() { const total = tasks.length, completed = tasks.filter(t=>t.completed).length; document.getElementById("stats").innerHTML = `<div>Total: ${total}</div><div>Active: ${total-completed}</div><div>Completed: ${completed}</div>`; }
export function renderTaskList(onToggle, onDelete, onEdit, onLog) {
    ...
    container.innerHTML = filtered.map(task => `<div class="task-item ${task.completed ? 'completed' : ''}" data-id="${task.id}"><div class="task-header"><input type="checkbox" class="task-select" data-id="${task.id}" ${selectedIds.has(task.id) ? 'checked' : ''}><span class="task-title">${escapeHtml(task.title)}</span><span class="badge">${task.priority}</span>${task.dueDate ? `<span class="badge"><i class="far fa-clock"></i> ${new Date(task.dueDate).toLocaleString()}</span>` : ''}${task.tags ? task.tags.map(t=>`<span class="badge tag">${escapeHtml(t)}</span>`).join('') : ''}<div class="task-actions"><button class="complete-btn" data-id="${task.id}"><i class="fas ${task.completed ? 'fa-undo' : 'fa-check'}"></i></button><button class="edit-btn" data-id="${task.id}"><i class="fas fa-edit"></i></button><button class="delete-btn" data-id="${task.id}"><i class="fas fa-trash"></i></button><button class="log-btn" data-id="${task.id}"><i class="fas fa-history"></i></button></div></div>${task.description ? `<div class="task-desc">${escapeHtml(task.description)}</div>` : ''}</div>`).join('');
