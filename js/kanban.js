...
export function renderKanban(tasks, onToggle, onEdit, onDelete) {
    ...
    const statusMap = { backlog: 'Backlog', inProgress: 'In Progress', done: 'Completed' };
    let html = '';
    for (const [key, columnTasks] of Object.entries(columns)) {
        html += `<div class="kanban-column"><h3>${statusMap[key]}</h3>`;
